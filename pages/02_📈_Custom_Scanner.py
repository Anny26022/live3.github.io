...
from sidebar import custom_sidebar
...
""", unsafe_allow_html=True)
    ...
    st.markdown("""
        ...
        .tiny-turnover-inputs .stNumberInput {
            ...
            border: 1.2px solid #33343a !important;
            box-shadow: none !important;
        }
    ...
    if not st.session_state.price_bands_loading and st.session_state.price_bands_df.empty:
        ...
        with st.spinner("Loading price bands..."):
            try:
                ...
                import sys
                fetch_price_bands = None
                price_bands_module_path = 'pages/09_💹_Price_Bands.py'
                spec = importlib.util.spec_from_file_location('price_bands_mod', price_bands_module_path)
                if spec is not None:
                    price_bands_mod = importlib.util.module_from_spec(spec)
                    sys.modules['price_bands_mod'] = price_bands_mod
                    try:
                        spec.loader.exec_module(price_bands_mod)
                        fetch_price_bands = getattr(price_bands_mod, 'fetch_price_bands', None)
                    except Exception:
                        fetch_price_bands = None
                if fetch_price_bands is None:
    ...
    if st.session_state['summary_choice_toggle'] == "Search Results":
        ...
        with copy_format_tab1:
            ...
            if not df.empty:
                ...
                st.markdown("<b>Select and copy the tickers below. Editing is disabled.</b>", unsafe_allow_html=True)
                st.code(simple_tickers, language=None)
                st.markdown("<span style='font-size:1.2em;' title='Copy'><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='none' viewBox='0 0 24 24'><rect width='16' height='16' x='4' y='4' rx='2' fill='#facc15' stroke='#eab308' stroke-width='1.5'/><path d='M8 8h8v8H8z' fill='#fef08a' stroke='#eab308' stroke-width='1.5'/></svg></span> <b>Click and press Ctrl+C to copy</b>", unsafe_allow_html=True)
            ...
            if not df.empty and 'industry' in df.columns:
                ...
                for _, row in industry_groups.iterrows():
                    if pd.notna(row['industry']) and row['industry']:
                        ...
                        line = f"{row['industry']}({row['count']}): " + ','.join(symbols)
                        industry_lines.append(line)
                industry_formatted = '\n'.join(industry_lines)
                st.markdown("<b>Select and copy the tickers below. Editing is disabled.</b>", unsafe_allow_html=True)
                st.code(industry_formatted, language=None)
                st.markdown("<span style='font-size:1.2em;' title='Copy'><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='none' viewBox='0 0 24 24'><rect width='16' height='16' x='4' y='4' rx='2' fill='#facc15' stroke='#eab308' stroke-width='1.5'/><path d='M8 8h8v8H8z' fill='#fef08a' stroke='#eab308' stroke-width='1.5'/></svg></span> <b>Click and press Ctrl+C to copy</b>", unsafe_allow_html=True)
            elif not df.empty:
        ...
        with copy_format_tab3:
            ...
            if not df.empty and 'sector' in df.columns:
                ...
                for _, row in sector_groups.iterrows():
                    if pd.notna(row['sector']) and row['sector']:
                        symbols = [f"NSE:{symbol}" for symbol in row['name']]
                        line = f"{row['sector']}({row['count']}): " + ','.join(symbols)
                        sector_lines.append(line)
                sector_formatted = '\n'.join(sector_lines)
                st.markdown("<b>Select and copy the tickers below. Editing is disabled.</b>", unsafe_allow_html=True)
                st.code(sector_formatted, language=None)
                st.markdown("<span style='font-size:1.2em;' title='Copy'><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='none' viewBox='0 0 24 24'><rect width='16' height='16' x='4' y='4' rx='2' fill='#facc15' stroke='#eab308' stroke-width='1.5'/><path d='M8 8h8v8H8z' fill='#fef08a' stroke='#eab308' stroke-width='1.5'/></svg></span> <b>Click and press Ctrl+C to copy</b>", unsafe_allow_html=True)
            elif not df.empty:
